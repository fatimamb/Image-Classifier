...
def train_classifier(model, optimizer, loss_fn, epochs, t_images, t_labels, v_images, v_labels, gpu):
 model.to(gpu)
 for epoch in range(epochs):
    ...
    t_images, t_labels = t_images.to(gpu), t_labels.to(gpu)
        #Feedforawrd
    pred = model.forward(t_images)
    loss = loss_fn(pred, t_labels)
     #accourcy:
    accurcy = loss_fn(pred, t_labels) #loss
  ...
  #Track the loss and accuracy on the validation set to determine the best hyperparameters
    with torch.no_grad():
        ...
        model.eval()
        # Move images and label tensors to the default gup
        v_images, v_labels = v_images.to(gpu), v_labels.to(gpu)
        pred=model(v_images)
...
def test(model, s_images, s_labels, gpu):
    with torch.no_grad():
       #loss
        # set model with out dropout:
        model.eval()
        # Move images and label tensors to the default gup
        s_images, s_labels = s_images.to(gpu), s_labels.to(gpu)
        #accurcy:
...
def save_checkpoint(model, image_datasets_training, input_size,arch):
